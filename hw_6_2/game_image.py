...
class game_image(game_image_base):
    ...
    def transform_image(self, name_method: str):

        '''
        викликає приватний метод класу

        перетворює поточне значення змінної self.__img в масив та
        викликає приватний метод класу який виконує трансформацію
        зображення

        :param name_method: назва методу.
        :exception AttributeError: якщо метод не визначено.
        '''

        self.__img_array = np.asarray(self.__img)
        try:
            getattr(self, '_game_image' + name_method)()
    ...
    def __mirror_vertical(self) -> None:
    ...
    def __mirror_horizon(self) -> None:
